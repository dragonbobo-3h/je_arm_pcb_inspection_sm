...
struct StInspectLeftGripperOpen;

struct StInspectRightGripperClose : smacc2::SmaccState<StInspectRightGripperClose, StInspect>
{
  using SmaccState::SmaccState;

  typedef boost::mpl::list<
    smacc2::Transition<smacc2::EvCbSuccess<cl_moveit2z::CbCtrlGripper, OrGripper>, StInspectLeftGripperOpen>,
    smacc2::Transition<smacc2::EvCbFailure<cl_moveit2z::CbCtrlGripper, OrGripper>, StPause>,
    smacc2::Transition<EvGripperClosed, StInspectLeftGripperOpen>,
    smacc2::Transition<EvPauseRequested, StPause>
  > reactions;

  static void staticConfigure()
  {
    const auto cfg = je_arm_pcb_inspection_sm::utils::loadGripperCommandConfig("inspect_right_close");
  ...
  void onEntry()
  {
    this->setGlobalSMData(std::string(sm_data::kInspectResumeSubstateId), std::string(sm_data::kInspectSubstateRightGripperClose));
    this->setGlobalSMData(std::string(sm_data::kWorkResumeSubstateId), std::string(sm_data::kWorkSubstateInspect));
    RCLCPP_INFO(getLogger(), "WORK::INSPECT::RIGHT_GRIPPER_CLOSE - waiting right grip feedback [n bypass]");
  }
};
