...
struct StInspectRightRetreat;

struct StInspectLeftGripperOpen : smacc2::SmaccState<StInspectLeftGripperOpen, StInspect>
{
  using SmaccState::SmaccState;

  typedef boost::mpl::list<
    smacc2::Transition<smacc2::EvCbSuccess<cl_moveit2z::CbCtrlGripper, OrGripper>, StInspectRightRetreat>,
    smacc2::Transition<smacc2::EvCbFailure<cl_moveit2z::CbCtrlGripper, OrGripper>, StPause>,
    smacc2::Transition<EvGripperOpened, StInspectRightRetreat>,
  ...
  void onEntry()
  {
    this->setGlobalSMData(std::string(sm_data::kInspectResumeSubstateId), std::string(sm_data::kInspectSubstateLeftGripperOpen));
    this->setGlobalSMData(std::string(sm_data::kWorkResumeSubstateId), std::string(sm_data::kWorkSubstateInspect));
    RCLCPP_INFO(getLogger(), "WORK::INSPECT::LEFT_GRIPPER_OPEN - release from left, waiting feedback [n bypass]");
  }
};
